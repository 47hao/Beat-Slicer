...
class Calibration(Mode):
    def appStarted(mode):
        ...
        mode.reButtonPos = (mode.width*.35, mode.height*.87)
        ...
        mode.contButtonPos = (mode.width*.65, mode.height*.87)
    ...
    def drawUI(mode, canvas):
        f = "Teko 48 bold"
        canvas.create_text(mode.width/2,mode.height*.1,text="CALIBRATION",font=f,fill="white")
        f = "Teko 24 bold"
        t = "MOVE COMFORTABLY TO SET YOUR RANGE OF MOTION"
        canvas.create_text(mode.width/2,mode.height*.15,text=t,font=f,fill="gray")
